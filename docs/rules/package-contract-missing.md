...
2. a local override at `.solid-check/contracts/<package>/solid-reactivity.json`,
3. `solid-reactivity.json` shipped in the package's own root,
4. the contracts bundled with solid-check (`solid-js`, `@solidjs/web`).
...
return accessors. Without a contract describing its exports, solid-check cannot see
...
- **You consume the package** — create a local contract at
  `.solid-check/contracts/<package>/solid-reactivity.json` (the path the finding
